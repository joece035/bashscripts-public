...
# Backward-compat alias: gclean! → gcleanx (so old muscle memory still
# hits the safe path; new name makes the danger explicit)
alias gclean!='gcleanx' 2>/dev/null
# Make it a function instead of alias so it appears in declare -f
gclean!() { gcleanx "$@"; }
